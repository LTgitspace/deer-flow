---
title: 运维与排障
description: 本页面涵盖运行 UniDeer 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 UniDeer 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

UniDeer 应用在 `logs/` 目录中写入每个服务的日志：
...
```bash
# 检查线程用户数据目录是否存在且可写
ls -la backend/.unideer/threads/
```

**解决**：

1. 确保 `backend/.unideer/` 对运行 UniDeer 的进程可写。
2. 在 Docker 部署中，验证卷挂载路径正确（`UNI_DEER_ROOT` 设置为绝对路径）。
...
**原因**：SQLite 支持并发读取，但同一时间只有一个写入者。UniDeer 的检查点写入和应用写入会共享同一个 SQLite 数据库文件，多用户并发时可能竞争文件锁。
...
- `backend/.unideer/threads/`：线程工作目录、上传文件和产出物。
- `backend/.unideer/memory.json` 和 `backend/.unideer/agents/`：全局和 Agent 记忆。
- `${database.sqlite_dir}/unideer.db`：当使用 `database.backend: sqlite` 时备份，默认路径为 `.unideer/data/unideer.db`。
