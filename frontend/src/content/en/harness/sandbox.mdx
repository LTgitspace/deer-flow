...
<Callout type="info" emoji="📦">
  ...
  command-based work. It is what makes UniDeer capable of real action, not just
  conversation.
</Callout>
...
UniDeer supports four sandbox modes. Choose the one that fits your deployment:
...
sandbox:
  ...
  # Optional: container name prefix (default: unideer-sandbox)
  container_prefix: unideer-sandbox
...
```yaml
sandbox:
  use: unideer.community.e2b_sandbox:E2BSandboxProvider
...
Install: `e2b-code-interpreter` is already a core dependency of `unideer-harness`, so no extra install step is needed — just provide your API key and switch the provider in `config.yaml`.
...
<Callout type="info">
  Because e2b is a remote sandbox, UniDeer's `/mnt/user-data` virtual prefix is
...
| `.unideer/threads/{thread_id}/user-data/` | `/mnt/user-data/`                            | Read-write |
...
provider runs inside the `unideer-gateway` container. In that mode,
...
services:
  gateway:
    volumes:
      - ${DEER_FLOW_REPO_ROOT}/.unideer/knowledge:/app/.unideer/knowledge:ro
```
...
sandbox:
  use: unideer.sandbox.local:LocalSandboxProvider
  mounts:
    - host_path: /app/.unideer/knowledge
      container_path: /mnt/knowledge
      read_only: true
```

If the configured `host_path` is not visible to the gateway process, UniDeer
