---
title: Customization
description: UniDeer's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
---
...
<Callout type="info" emoji="🔧">
  UniDeer is designed to be adapted. You can extend agent behavior by writing
...
UniDeer's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
...
from unideer.agents.thread_state import ThreadState
...
```python
from unideer.sandbox.sandbox_provider import SandboxProvider
from unideer.sandbox.sandbox import Sandbox
...
```python
from unideer.agents.memory.storage import MemoryStorage
...
For custom backends, implement the LangGraph `BaseCheckpointSaver` interface and configure it programmatically when initializing the `UniDeerClient`.
...
guardrails:
  enabled: true
  provider:
    use: unideer.guardrails.builtin:AllowlistProvider
    config:
