...
description: Why tool calls left in a safety-terminated model response must not be executed, and how to configure provider detectors in UniDeer.
date: 2026-05-22
tags:
  - Safety
  - Agents
...
## What UniDeer Does at This Boundary
...
When `detectors` is not configured explicitly, UniDeer uses the built-in detector set:
...
This default set covers common UniDeer paths for OpenAI-compatible providers, Anthropic, and Gemini. It does not treat a normal `finish_reason="tool_calls"` as a safety termination, and it does not fold length truncation such as `length` or `max_tokens` into the safety category.
...
safety_finish_reason:
  enabled: true
  detectors:
    - use: unideer.agents.middlewares.safety_termination_detectors:OpenAICompatibleContentFilterDetector
      config:
        finish_reasons: ["content_filter", "sensitive"]

    - use: unideer.agents.middlewares.safety_termination_detectors:AnthropicRefusalDetector

    - use: unideer.agents.middlewares.safety_termination_detectors:GeminiSafetyDetector
```
