...
export function warnUnsupportedStreamModes(
  ...
  warn(
    `[unideer] Rejected unsupported LangGraph stream mode(s): ${unseenModes.join(", ")}`,
  );
}
...
export function sanitizeRunStreamOptions<T>(options: T): T {
  ...
  if ("streamResumable" in options) {
    ...
    if (!warnedUnsupportedStreamResumable) {
      ...
      console.warn(
        "[unideer] Dropped unsupported LangGraph run option: streamResumable",
      );
    }
  }
  ...
  if (droppedModes.length > 0) {
    ...
    throw new Error(
      `[unideer] Unsupported LangGraph stream mode(s): ${droppedModes.join(", ")}`,
    );
  }
