"""Tests for BusinessRequirementMiddleware's forced skill-context gate."""
...
from deerflow.agents.middlewares.business_requirement_middleware import (
    _SKILL_NAME,
...
def _slash_reminder(skill_name: str = "business-requirement") -> HumanMessage:
    return HumanMessage(
        content=(
...
def _run(messages: list, state: dict | None = None) -> ModelRequest:
    middleware = BusinessRequirementMiddleware()
...
def test_inactive_skill_passes_through_untouched() -> None:
    # Regression: previously BRD nudges fired on every conversation, even "hi".
