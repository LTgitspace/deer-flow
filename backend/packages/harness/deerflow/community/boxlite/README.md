...
Runs each UniDeer sandbox as a [BoxLite](https://github.com/boxlite-ai/boxlite)
...
The `boxlite` package is an optional UniDeer harness extra, not part of the
...
hosts, such as Windows, should use another sandbox provider or run UniDeer from
a supported Linux/macOS environment.

**Host requirement:** BoxLite boots micro-VMs, so a Linux host needs KVM — i.e.
nested virtualization when UniDeer runs inside a cloud VM. macOS uses
Hypervisor.framework. This is the main deployment constraint to weigh vs. the
container-based providers.

## Design

UniDeer's `Sandbox` contract is synchronous; BoxLite's SDK is async-native and
