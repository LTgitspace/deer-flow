...
UniDeer supports single sign-on (SSO) via any OpenID Connect (OIDC) 2.0 compliant provider. This includes Keycloak, Google Workspace, Azure AD, Okta, and many others.
...
- **Existing UniDeer JWT** — After successful OIDC authentication, UniDeer creates its own JWT session cookie. The OIDC provider's tokens are never exposed to the browser.
...
providers:
  <provider-id>:
    ...
    # User provisioning
    auto_create_users: true         # Auto-create UniDeer account on first SSO login (default: true)
...
5. After successful authentication, you'll be redirected back to the UniDeer workspace
...
- Password changes must be done through the SSO provider, not UniDeer
