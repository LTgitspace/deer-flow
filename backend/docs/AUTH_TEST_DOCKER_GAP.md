...
|---|---|---|---|
| TC-DOCKER-01 | `deerflow.db` volume persistence | Verify the `UNI_DEER_HOME` bind mount survives container restart | needs `docker compose up` |
...
# Optional: pin UNI_DEER_HOME to a stable host path
echo "UNI_DEER_HOME=$HOME/deer-flow-data" >> .env
```
