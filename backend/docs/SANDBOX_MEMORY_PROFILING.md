...
compare candidates with the same UniDeer workload.
...
same UniDeer workload has been measured on both the current AIO sandbox and the
candidate backend.

For an experimental provider PR, prefer `Related to #3213` unless the PR also
includes reproducible UniDeer workload data that demonstrates the target memory
