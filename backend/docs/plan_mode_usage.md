...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in UniDeer 2.0.
...
UniDeer uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall UniDeer prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with UniDeer's main prompt
...
- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom UniDeer-style prompts**
...
- Custom prompts emphasize the same principles as UniDeer's main system prompt (clarity, action-oriented, critical rules)
