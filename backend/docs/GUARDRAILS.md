...
The simplest option. Ships with UniDeer. Block or allow tools by name. No external packages, no passport, no network.
...
Any provider that accepts `framework` as a kwarg and implements `evaluate`/`aevaluate` works. The OAP standard defines the passport format and decision codes; UniDeer doesn't care which provider reads them.
...
A custom OAP provider can implement any evaluation strategy -- the UniDeer middleware doesn't care how the provider reaches its decision.

**Try it:**
1. Install and set up as above
2. Start UniDeer and ask: "Create a file called test.txt with content hello"
...
2. Add the config
3. Start UniDeer and ask: "Use bash to delete test.txt"
...
| Field | Example use |
|---|---|
| `user_id` | Attach the authenticated UniDeer user to a provider-side policy or audit record |
...
class ContextAwareGuardrailProvider:
    ...
    async def aevaluate(self, request):
        ...
        # (UniDeer enforces a blocking-IO gate in CI). If your policy
    ...
    def _decide(self, request):
        # 1. Normalize UniDeer request data into policy context.
        context = {
        ...
        # 3. Convert the policy result back to UniDeer's decision object.
        return GuardrailDecision(
            allow=result["allow"],
            reasons=[
                GuardrailReason(
...
UniDeer loads providers via `resolve_variable()` -- the same mechanism used for models, tools, and sandbox providers. The `use:` field is a Python class path: `package.module:ClassName`.
